...
public:
  ...
  void passenger_board();  // person tries to board the bus increments bus_count by one if bus_count < capacity
  void passenger_exit();   // person exits bus - decrements bus_count by one if bus has more than zero people.
